...
def main():
    ...
    for csv_path in root_dir.glob('*.csv'):
        ...
        df.drop( df[df['ResponseAngle'] == 0].index, inplace=True)     # Drop responses at midline

        df = df[df['SpkrPos'].abs() == 90]      # Select only left and right speakers
        df = df[df['ResponseAngle'].abs() == 90]      # Select only left and right response spouts
    ...
    # plt.show()
    save_path = 'Results/Localization/images/left_vs_right.png'
