function step2_crossref_temp_and_behavior_ProperGraph(rootDir)
...
tempDir  = fullfile(rootDir,'Temperature Records - Editted');
...
for i = 1 : nFiles
   ...
   Ti.UniversalStartTime = seconds2days(Ti.StartTime) + offset;   
...
% Average temperature in window around trial onset
tWindow = -2 : 2; 
tWindow = seconds2days(tWindow);

for i = 1 : numel(tWindow)
    refTime(:,i) = B.UniversalStartTime + tWindow(i);
end

% Interpolate loop temperatures at times of trial onset
LeftTemperature  = interp1(T.datetime, T.Loop_L, refTime);
RightTemperature = interp1(T.datetime, T.Loop_R, refTime);

B.LeftTemperature  = mean(LeftTemperature, 2);
B.RightTemperature = mean(RightTemperature, 2);

% Assume that missing values indicate warm data
...
% Cooling Data
% plot(T.datetime,T.ambient_R,'--or')
% plot(T.datetime,T.ambient_L,'--ob')
plot(T.datetime,T.Loop_R,'r')
plot(T.datetime,T.Loop_L,'b')
...
for mask = 0 : 2
    ...
    % Plot data
    s(mask+1,1) = scatter(M.UniversalStartTime, M.LeftTemperature, 'marker', marker{mask+1});
    s(mask+1,2) = scatter(M.UniversalStartTime, M.RightTemperature,'marker', marker{mask+1});
end

set(s(:,1),'MarkerEdgeColor','b')
set(s(:,2),'MarkerEdgeColor','r')

lh = legend(s(:,2),'No noise','Restricted','Continuous');
set(lh,'Location','eastoutside','edgeColor','none')
